...
function getButtonLabel(progressPercent: number) {
  ...
  if (progressPercent >= 90) {
    return "Selesaikan Materi";
  }

  return "Lanjutkan Belajar";
}

export function TopicCatalogView({
  displayName,
  homeHref,
  dashboardData,
  isEnrolled = false,
  logoutAction,
}: TopicCatalogViewProps) {
            ...
            <section className="rounded-[24px] border border-[#d8e2f3] bg-[#f5f8ff] px-6 py-7 shadow-[0_20px_42px_-36px_rgba(15,23,42,0.45)] lg:px-8">
              <div className="max-w-[820px]">
                ...
                <p className="mt-4 max-w-[780px] text-[16px] leading-9 text-[#596983]">
                  Temukan beragam materi pembelajaran interaktif untuk membantumu meraih prestasi terbaik.
                </p>
              </div>
                ...
                <div className="mt-8 grid gap-6 xl:grid-cols-[256px_minmax(0,1fr)]">
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    {dashboardData.topics.map((topic, index) => {
                      ...
                      const buttonLabel = getButtonLabel(progressPercent);
                      ...
                      return (
                        ...
                        >
                          <div className={cn("relative h-[196px] overflow-hidden", theme.mediaClassName)}>
                            ...
                            <div className="absolute bottom-0 left-0 right-0 h-[56%] bg-[linear-gradient(180deg,transparent,rgba(15,23,42,0.2))]" />
                          </div>
                          ...
                          <div className="px-6 py-6">
                            ...
                            <div className="mt-5 flex items-center justify-between gap-3 rounded-[16px] bg-[#f8fbff] px-4 py-3">
                              <div className="flex items-center gap-2 text-[13px] font-medium text-[#596983]">
                                <Lock className="size-4" />
                                {isPreview ? "Bisa dibuka tanpa daftar" : "Butuh akun siswa untuk akses penuh"}
                              </div>
                            </div>

                            <div className="mt-5">
                              <Link
                                href={isPreview ? `${topicHref}/${topic.slug}` : "/siswa"}
                                className={cn(
                                  "flex h-[48px] items-center justify-center rounded-[12px] text-[15px] font-bold",
                                  progressPercent <= 0
                                    ? "border-2 border-[#1f57f3] bg-white text-[#1f57f3]"
